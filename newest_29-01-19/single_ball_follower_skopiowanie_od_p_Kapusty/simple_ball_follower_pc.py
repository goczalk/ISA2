...
while True:
    ...
    if not paused:
        ...
        if a_button_clicked:
            ...
            circles = cv2.HoughCircles(gray_cropped, cv2.HOUGH_GRADIENT, 1, 1000, param1=100, param2=30, minRadius=15,
                                       maxRadius=800)
            if debugBallIn:
                print("ballIn")
        ...
        cv2.rectangle(upscaledColor, (xROI, yROI), (xROI + roiSize[0] * scaleFactor, yROI + roiSize[1] * scaleFactor),
                      (0, 0, 0), thickness=3)
    ...
    if key == ord('q'):
        ...
        # for h in range(10,30,10):
        #     for s in range(10, 60, 10):
        #         for v in range(10, 80, 10):
        #             print("MODLOW: HUE:{}, SAT:{}, VAL:{}".format(avg_h - h, avg_s - s, avg_v - v))
        #             print("MODHIG: HUE:{}, SAT:{}, VAL:{}".format(avg_h + h, avg_s + s, avg_v + v))
        #             colorLower = (max(0, avg_h - h), max(0, avg_s - s), max(0, avg_v - v))
        #             colorUpper = (min(255, avg_h + h), min(255, avg_s + s), min(255, avg_v + v))
        #             mask1 = cv2.inRange(resizedHSV, colorLower, colorUpper)
